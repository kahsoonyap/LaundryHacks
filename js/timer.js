...
var d = firebase.database().ref().child("wm1");
...
function changeTime(){
	...
	if (washingTimer < 0){
		...
		wm.innerHTML = "0 seconds left in cycle";
		wm.addEventListener("click", stop);
	} else {
		if (washingTimer == 1){
			wm.innerHTML = washingTimer + " second left in cycle";
		} else {
			wm.innerHTML = washingTimer + " seconds left in cycle";
		}
	}
}
